<x-app-layout>
    <x-head-lable backhref="{{ route('workshop-category.index') }}">
        {{ __('Workshop Category Edit') }}
    </x-head-lable>

    <form method="POST" action="{{ route('workshop-category.update', $workshopCategory->id) }}"
        ...
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div class="flex items-center space-x-1">
                ...
                <x-text-input id="name" class="block mt-1 w-full" type="text" name="name" :value="old('name', $workshopCategory->name)"
        ...
        <div class="flex items-center justify-end mt-4">
            <a class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-200 focus:ring-4 focus:ring-gray-100 font-medium rounded-full text-sm px-5 py-2.5 me-2 mb-2 "
                href="{{ route('workshop-category.index') }}">
                {{ __('Cancel') }}
            </a>
