<x-app-layout>
    <x-head-lable backhref="{{ route('meditation-audio.index') }}">
        {{ __('Meditation Audio Edit') }}
    </x-head-lable>

    <form method="POST" action="{{ route('meditation-audio.update', $meditationAudio->id) }}"
...
<div class="mt-4">
    ...
    <!-- 👇 Give ID to update preview -->
    <img id="audio_thumb_preview" src="{{ $meditationAudio->audio_thumb }}" alt="thumb upload" class="w-16 my-2">
</div>

<!-- inner_thumb -->
<div class="mt-4">
    <div class="flex items-center space-x-1">
        <x-input-label for="inner_thumb" :value="__('Audio Thumb (400x480)')" />
        <span class="text-red-500">*</span>
    </div>
    ...
    <!-- 👇 Give ID to update preview -->
    <img id="inner_thumb_preview" src="{{ asset($meditationAudio->inner_thumb) }}" alt="thumb upload" class="w-16 my-2">
</div>
            ...
            <div class="mt-4">
                ...
                <select id="premium_type" name="premium_type"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option value="1" @selected(old('premium_type', $meditationAudio->premium_type) == '1')>true</option>
                    <option value="0" @selected(old('premium_type', $meditationAudio->premium_type) == '0')>false</option>
                </select>
            </div>
            ...
            <div class="mt-4">
                ...
                <textarea id="short_description" name="short_description" rows="4"
                    ...
                    placeholder="Enter short description...">{{ old('short_description', $meditationAudio->short_description) }}</textarea>
            ...
            <div class="mt-4">
                ...
                <textarea id="description" name="description" rows="4"
                    class="block mt-1 w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-200"
                    placeholder="Enter description...">{{ old('description', $meditationAudio->description) }}</textarea>
    ...
    <script>
    // Audio Thumb Live Preview
    document.getElementById('audio_thumb').addEventListener('change', function (e) {
        const file = e.target.files[0];
        if (file) {
            const preview = document.getElementById('audio_thumb_preview');
            preview.src = URL.createObjectURL(file);
        }
    });

    // Inner Thumb Live Preview
    document.getElementById('inner_thumb').addEventListener('change', function (e) {
        const file = e.target.files[0];
        if (file) {
            const preview = document.getElementById('inner_thumb_preview');
            preview.src = URL.createObjectURL(file);
        }
    });
</script>
